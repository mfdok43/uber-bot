...
export class SaveChannel {
    ...
    init = async () => {
        try {
            ...
            let channelTitle = this.channelTitle


            await this.channelModel.sync();
            this.foundChannel = await this.channelModel.findOne({ where: { telegramId } }).then((res: any) => {
                if (res != null) {
                    ...
                    this.channelModel.create({ telegramId: telegramId, channelName: channelName, channelTitle: channelTitle });
                }
            });
        } catch (e) {
            console.log(e)
...
export class GetChannelsByAdmin {
    ...
    async init (value: boolean = false) {
        try {
            this.channels = await this.channelModel.findAll({
                raw: true,
                where: { isAdmin: value },
